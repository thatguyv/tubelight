...
export async function generateFlashcards(input: SectionInput): Promise<Flashcard[]> {
  const result = await runJsonPrompt<{ flashcards: Flashcard[] }>({
    ...
    user: [
      header(input.meta, input.outputLanguage),
      "Task: Generate 8-15 Anki-style flashcards covering the most important concepts.",
      "Rules:",
      "- Questions are atomic and focused (one fact per card).",
      "- Answers are 1-3 sentences. No 'in the video' phrasing — phrase as standalone facts.",
      "- Difficulty: 'easy' for basic facts, 'medium' for explanations, 'hard' for synthesis.",
...
export async function generateMindMap(input: SectionInput): Promise<MindMapNode> {
  const result = await runJsonPrompt<{ mindmap: MindMapNode }>({
    ...
    user: [
      header(input.meta, input.outputLanguage),
      "Task: Build a hierarchical mind map of the video's ideas, 2-3 levels deep.",
      "Root label = video topic. Each node has 2-6 children. Keep labels SHORT (2-7 words).",
      'Schema: { "mindmap": { "label": string, "children": [ { "label": string, "children": [...] } ] } }',
      "",
      "TRANSCRIPT:",
      bodyText(input.transcript),
    ].join("\n"),
